...
pub struct ConfidentialStreamVault {
    ...
    /// Total assets at last checkpoint (excludes streaming yield not yet accrued)
    pub base_assets: u64,
    /// Current shares outstanding (tracked in plaintext for aggregate accounting)
    pub total_shares: u64,
    /// Yield to distribute over current stream period
    pub stream_amount: u64,
    /// Unix timestamp when current stream started
    ...
    /// Optional auditor ElGamal public key for regulatory compliance
