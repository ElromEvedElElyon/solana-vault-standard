...
//!
//! Combines SVS-5 streaming yield distribution with SVS-3 confidential transfers.
//! Share balances are encrypted on-chain via Token-2022 Confidential Transfers.
//! Yield accrues linearly over time (streaming model), with aggregate vault
//! state (base_assets, total_shares, stream params) in plaintext.
...
declare_id!("SVS6ConfStreamVau1tXXXXXXXXXXXXXXXXXXXXXXXXX");

#[program]
pub mod svs_6 {
    use super::*;

    /// Initialize a new confidential streaming vault for the given asset
    ...
    /// Deposit assets and receive confidential shares at streaming price
    ...
    /// Mint exact confidential shares by depositing required assets at streaming price
    ...
    /// Withdraw exact assets by burning confidential shares at streaming price
    /// Requires pre-verified ZK proof context accounts
    pub fn withdraw(
    ...
    /// Redeem confidential shares for assets at streaming price
    /// Requires pre-verified ZK proof context accounts
    pub fn redeem(
        ctx: Context<Redeem>,
        shares: u64,
    ...
    /// Start a new yield stream over a specified duration
    /// Authority deposits yield tokens that accrue linearly to share price
    ...
    /// Permissionless checkpoint: finalize accrued yield into base_assets
    /// Can be called by anyone (MEV bots, keepers, users)
    pub fn checkpoint(ctx: Context<CheckpointAccounts>) -> Result<()> {
    ...
    /// Max assets owner can withdraw (vault total, encrypted balances unreadable)
    pub fn max_withdraw(ctx: Context<VaultView>) -> Result<()> {
        instructions::view::max_withdraw(ctx)
    }

    /// Max shares owner can redeem (u64::MAX, encrypted balances unreadable)
