//! Initialize instruction: create MultiAssetVault PDA and shares mint (Token-2022).
...
use crate::{
    constants::{BASE_DECIMALS, MAX_DECIMALS, SHARES_DECIMALS, SHARES_MINT_SEED, VAULT_SEED},
    error::VaultError,
...
pub fn handler(ctx: Context<Initialize>, vault_id: u64, base_decimals: u8) -> Result<()> {
    require!(base_decimals <= MAX_DECIMALS, VaultError::InvalidAssetDecimals);
    ...
    vault.decimals_offset = MAX_DECIMALS.checked_sub(base_decimals).ok_or(VaultError::MathOverflow)?;
    vault.bump = vault_bump;
    vault.paused = false;
    vault.vault_id = vault_id;
    vault.num_assets = 0;
    vault.base_decimals = if base_decimals == 0 { BASE_DECIMALS } else { base_decimals };
